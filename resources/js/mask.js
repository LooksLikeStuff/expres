    ...
    setTimeout(function() {
        setTimeout(function() {
            ...
            if (element) { // Добавляем проверку
                ...
                console.log('Элемент .some-element не найден');
            }
        }, 100);
    }, 100);
});
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
        ...
        if (element) { // Добавляем проверку
            ...
            console.log('Элемент .some-element не найден во втором обработчике');
        }
    }, 100);
});
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
        ...
        if (element) {
            ...
            console.log('Элемент .some-element не найден в третьем обработчике');
        }
    }, 100);
});
...
const safeApplyStyle = (selector, styleProperty, value) => {
    ...
    if (element && element.style) {
        ...
        console.warn(`Элемент ${selector} не найден или не имеет свойства style`);
        return false;
    }
};

// Глобальная функция для безопасной работы с DOM элементами
window.safeElementOperation = (selector, operation, ...args) => {
    try {
        const element = typeof selector === 'string' ? document.querySelector(selector) : selector;
        if (element) {
            if (typeof operation === 'function') {
                return operation(element, ...args);
            } else if (typeof operation === 'string' && element[operation]) {
        ...
        console.error(`Ошибка при выполнении операции с элементом ${selector}:`, error);
    }
    return null;
};

// Безопасная функция для работы со стилями
window.safeStyleSet = (selector, styles) => {
    return window.safeElementOperation(selector, (element) => {
        if (element.style) {
